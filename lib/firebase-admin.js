...
if (missingFields.length > 0) {
  ...
  console.error('Firebase not configured - missing or empty:', missingFields);
} else {
  try {
    // Initialize without checking getApps() for server-side
    app = initializeApp(firebaseConfig, `admin-${Date.now()}`);
    ...
    try {
      ...
      console.error('Failed to use existing app:', fallbackError.message);
    }
  }
}
...
class FirebaseAdminWrapper {
  ...
  collection(name) {
    if (!firestore) {
      console.warn(`Firestore not initialized when accessing collection: ${name}`);
    }
...
class CollectionWrapper {
  ...
  async get() {
    if (!this.collectionRef) {
      console.warn('Collection not initialized, returning empty result');
      return {
        empty: true,
        size: 0,
        docs: [],
    ...
    try {
      ...
      return {
        ...
        docs: querySnap.docs.map(doc => ({
          id: doc.id,
          ref: new DocumentWrapper(doc.ref, doc.id, this.name),
          data: () => doc.data(),
          exists: doc.exists()
        })),
    ...
    try {
      ...
      return {
        ...
        docs: querySnap.docs.map(doc => ({
          ...doc,
          id: doc.id,
          ref: new DocumentWrapper(doc.ref, doc.id, this.fullPath),
          data: () => doc.data(),
          exists: doc.exists()
        })),
        forEach: (callback) => querySnap.forEach(callback)
    ...
    try {
      ...
      return {
        ...
        docs: querySnap.docs.map(doc => ({
          ...doc,
          id: doc.id,
          ref: new DocumentWrapper(doc.ref, doc.id),
          data: () => doc.data(),
          exists: doc.exists()
        })),
        forEach: (callback) => querySnap.forEach(callback)
